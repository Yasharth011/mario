...
struct rerun::CollectionAdapter<rerun::Vector3D, std::vector<Eigen::Vector3f>> {
    /// Borrow for non-temporary.
    Collection<rerun::Vector3D> operator()(const std::vector<Eigen::Vector3f>& container) {
        return Collection<rerun::Vector3D>::borrow(container.data(), container.size());
    }

    // Do a full copy for temporaries (otherwise the data might be deleted when the temporary is destroyed).
    Collection<rerun::Vector3D> operator()(std::vector<Eigen::Vector3f>&& container) {
        std::vector<rerun::Vector3D> positions(container.size());
        memcpy(positions.data(), container.data(), container.size() * sizeof(Eigen::Vector3f));
        return Collection<rerun::Vector3D>::take_ownership(std::move(positions));
    }
};
